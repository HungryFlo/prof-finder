...
from ..schemas import MessageResponse, SourceInputArxivCreate, SourceInputResponse
...
@router.get("", response_model=List[SourceInputResponse])
...
def create_pdf_source_input(
    ...
    try:
        ...
        markdown = extract_markdown_from_pdf(temp_path)
...
def create_arxiv_source_input(
    ...
    try:
        ...
        temp_pdf = download_to_temp_file(source_input.pdf_url)
        source_input.downloaded_pdf_path = str(temp_pdf)
        source_input.extracted_markdown = extract_markdown_from_pdf(temp_pdf)
