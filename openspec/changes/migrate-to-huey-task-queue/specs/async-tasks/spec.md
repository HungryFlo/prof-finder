## MODIFIED Requirement: Task State Management
...
## ADDED Requirement: Persistent Task Queue
...
## REMOVED Requirement: In-Memory Only Task Storage

~~系统 SHALL 在内存中管理任务状态（不持久化）。~~

替换为上述 MODIFIED Requirement: Task State Management（内存 + DB 双写）。
