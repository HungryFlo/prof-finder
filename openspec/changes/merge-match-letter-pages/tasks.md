...
- [ ] 1.2 Run `openspec validate merge-match-letter-pages --strict --no-interactive`
...
## 5. Validation
- [ ] 5.1 手动验证：Match 页面详情弹窗中邮件生成、编辑、复制、保存功能正常
- [ ] 5.2 手动验证：侧边栏不再显示「联络邮件」
- [ ] 5.3 手动验证：直接访问 `/letter` 路由时正确重定向
- [ ] 5.4 Run `openspec validate merge-match-letter-pages --strict --no-interactive`
