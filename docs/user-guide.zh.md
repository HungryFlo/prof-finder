...
1. 点击左侧「信息池」→「新建信息池」。
2. 按阶段完成：**脑暴**写下经历线索 → **聚类**归类整理 → **细化**补充过程细节 → **文书**勾选素材生成片段。
3. 可将文书片段应用到学生画像（同时绑定该信息池）。可创建多个独立信息池。

### 第四步：建立学生画像

1. 点击左侧「学生画像」→「上传简历」。
2. 选择 `.md`、`.tex`、`.txt` 等格式的简历文件；上传时可选择绑定一个信息池。
3. 建议开启 LLM 自动提取，系统会解析研究方向、技能、教育经历等字段。
4. 上传后可在详情页手动编辑，或用 AI 对话继续打磨画像。
...
2. 点击「生成邮件」，系统会根据你的画像、教授信息，以及绑定信息池中的细化经历生成个性化套磁信。
